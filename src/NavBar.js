...
const NavBar = () => {
  ...
  const formattedSubtotal = dollarUSLocale.format(subtotal);

  return (
    <div>
      <Navbar expand="md" className="py-3" fixed="top">
        <NavLink exact to="/" className="navbar-brand">
          Shoply
        </NavLink>
        <Nav className="ml-auto" navbar>
            <span>Items: {totalProducts}</span>
            <span className="subtotal">Subtotal: ${formattedSubtotal}</span>
          <NavItem>
